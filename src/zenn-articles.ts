...
async function _emojiFromGitHub(
  signal: AbortSignal,
  owner: string,
  repo: string,
  { title, link, filename }: FeedItem
): Promise<TitleItem> {
...
function emojiFromGitHub(
  [cancelPromise, cancel]: [Promise<void>, () => void],
  sendErr: ChanSend<any>,
  workerNum: number,
  owner: string,
  repo: string,
  items: ChanRecv<FeedItem>
): ChanRecv<TitleItem> {
  const ch = new Chan<Promise<TitleItem>>(workerNum - 1)
  ;(async () => {
    ...
    for await (const { title, link, filename } of breakGenerator(
      signal,
      items
    )) {
...
async function main(
  ...
  const errCh = new Chan<any>(2)
  let exitStatus = 0

  const recvItems = zennFeed(
    [cancelPromise, cancel],
    errCh.send,
    account,
    limit
  )
  const recvOut = emojiFromGitHub(
    [cancelPromise, cancel],
    errCh.send,
    workerNum,
    owner,
...
if (
  ...
  console.log(
    'USAGE: zenn-articles --acount [Zenn account] --owner [GitHub username] --repo [content repo] [--worker-num [number]] [--limit [number]] [--timeout [number]]'
  )
  process.exit(1)
}
