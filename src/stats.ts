#!/usr/bin/env zx
import { Chan, ChanSend } from 'chanpuru'
import { ProcessOutput, ProcessPromise } from 'zx'
import 'zx/globals'

$.verbose = false
function getSvg(sendErr: ChanSend<any>) {
  const cmds: (() => ProcessPromise<ProcessOutput>)[] = [
    () =>
  ...
  const ch = new Chan<ProcessPromise<ProcessOutput>>(1)
  ;(async () => {
    let aborted = false
