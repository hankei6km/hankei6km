...
async function _emojiFromGitHub(
  ...
  try {
    ...
    const emoji = (yaml.load(out.stdout) as any).emoji
    return {
      title: `${emoji} ${title}`,
      link,
      filename
...
function emojiFromGitHub(
  ...
  const ch = new Chan<Promise<FeedItem>>(workerNum - 1)
  ;(async () => {
...
async function main(
  ...
  const list: List = tree.children[0] as List
...
if (
  ...
  console.log(
    'USAGE: zenn-articles --acount [Zenn account] --owner [GitHub username] --repo [content repo] [--worker-num [number]] [--limit [number]]'
  )
  process.exit(1)
}
